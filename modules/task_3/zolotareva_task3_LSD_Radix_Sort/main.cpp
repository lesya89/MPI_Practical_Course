...
double  time_seq_work_alg_radix = 0;// Время последовательной версии
double  time_pp_work_alg_radix = 0;// параллельной версии поразрядной сортировки
...
for(int i = 0; i < size_arr; i++)
    arr[i] = rand()%100  - 50 + (double)(rand()) / RAND_MAX + 0.00000001;
return arr;
}
...
int offset;
      ...
      memset(counter, 0, sizeof(int) * 256); // Заполнить первые 256 байт нулями
for(int i = 0; i < size_arr; i++)
